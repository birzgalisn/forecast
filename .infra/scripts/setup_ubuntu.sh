...
curl -fsSL "https://pkgs.k8s.io/core:/stable:/v$KUBERNETES_BASE_VERSION/deb/Release.key" | gpg --dearmor -o /etc/apt/keyrings/kubernetes-apt-keyring.gpg
echo "deb [signed-by=/etc/apt/keyrings/kubernetes-apt-keyring.gpg] https://pkgs.k8s.io/core:/stable:/v$KUBERNETES_BASE_VERSION/deb/ /" | tee /etc/apt/sources.list.d/kubernetes.list
apt update
